...
src/app/api/analyze/  → GPT-4o-mini Vision 이미지 분석 + R2 업로드 + Supabase 로깅
src/app/api/search-products/ → 자체 DB 검색 (노드/attr 부스트, 한영 이중 키워드, description/color/material 확장 매칭)
...
| `scripts/crawl.ts` | 9개 플랫폼 크롤 CLI (--all, --site=, --probe=, --detail) |
...
| `supabase/migrations/001~010` | analyses, brand_nodes, products, eval_reviews, eval_golden_set, api_access_logs 스키마 |
