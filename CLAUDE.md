...
src/app/page.tsx      → 메인 — Q&A 에이전트 4단계 플로우 (레퍼런스/락/리파인/결과)
src/app/_qa/          → 메인의 단계 컴포넌트 + reducer + 휴리스틱 (Next.js _ prefix로 라우팅 제외)
...
| 파일 | 설명 |
|------|------|
| `src/app/page.tsx` | 메인 — Q&A 에이전트 useReducer 4단계 state machine (input/attributes/refine/results) + 키보드 네비 |
| `src/app/_qa/types.ts` | AgentState, LockableAttr, INITIAL_AGENT_STATE 정의 |
| `src/app/_qa/agent-reducer.ts` | reducer + AgentAction (테스트 가능) |
...
| `src/app/_qa/step-attributes.tsx` | Step 2 — 다중 item 선택 + 6개 속성 카드 + lock 체크박스 (max 2) + Detected 배너 + ★ Pick 배지 |
| `src/app/_qa/step-refine.tsx` | Step 3 — 스타일 감도 슬라이더 (라이브 결과 개수 프리뷰) + 가격 범위 + 재탐색 이유(라벨) |
| `src/app/_qa/step-results.tsx` | Step 4 — 결과 카드 그리드 (4-col) + lock 칩 오버레이 + 빈 결과 시 Unlock 추천 |
| `src/app/_qa/agent-progress.tsx` | 1/4 ~ 4/4 진행 인디케이터 (이전 단계 클릭 가능) |
| `src/lib/search/locked-filter.ts` | passesLockedFilter + toleranceToTargetCount (search-products와 테스트가 공유) |
