...
src/app/api/search-products/ → 자체 DB 검색 (노드/감도 부스트) + SerpApi fallback
...
src/lib/              → 유틸리티 (supabase.ts, fashion-genome.ts, prompts/)
...
| 상품 검색 | 자체 DB (Fashion Genome) + SerpApi fallback | 노드/감도 부스트 스코어링, DB 우선 |
...
| `src/app/api/search-products/route.ts` | 자체 DB (노드/감도 부스트 + 제외 필터) + SerpApi fallback |
...
| `supabase/migrations/001~008` | analyses, brand_nodes, products 스키마 + 제약조건 |
...
| 상품 검색 | 자체 DB (노드 부스트 + 감도 태그 부스트 + 제외 필터) + SerpApi fallback → 상위 5개 |
