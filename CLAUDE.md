...
src/app/api/search-products/ → 검색 엔진 v2 (product_ai_analysis enum 매칭 + 가중치 스코어링)
...
| 상품 검색 | product_ai_analysis enum 매칭 (subcategory 0.25 + colorFamily 0.20 + styleNode 0.30/0.15 + fit 0.15 + fabric 0.15 + moodTags 0.05×N) → 상위 5개, 브랜드당 max 2 |
