# GABI (가전비서 V2) UX 딥 분석 — kiko.ai Q&A Agent 개선 적용

> 작성일: 2026-04-13
> 목적: GABI의 UX 패턴을 kiko.ai `/agent` 4단계 MVP에 적용 가능한 개선안 도출
...
**kiko.ai 적용 시사점**: 현재 `/agent`의 AgentProgress는 상단 수평 스텝퍼로 데스크탑에 적합하지만, 모바일에서 4개 스텝 라벨이 좁아질 수 있음. GABI처럼 스텝 번호만 표시하고 라벨은 화면 제목으로 이동하는 것을 고려.
...
**핵심 UX**: 사용자가 이미 가지고 있는 맥락(견적서)을 출발점으로 삼는다. kiko.ai에서는 "레퍼런스 이미지"가 이에 해당.

**kiko.ai 대응** (Step 1 → Step 2):
...
- 단, kiko.ai는 이미 4단계라 더 늘리면 피로감. 대안: Step 3 내에서 섹션별 스크롤 + 각 섹션에 명확한 질문 헤딩
...
**kiko.ai 적용**: 장기 백로그. 패션은 가전보다 설명 복잡도가 낮아 음성 우선순위 낮음.

---

## 3. GABI vs kiko.ai 현재 MVP 갭 분석

| GABI 패턴 | kiko.ai 현재 (Step) | 갭 | 개선 난이도 |
|-----------|----------------------|-----|----------|
...
| Shape of AI 패턴 | GABI 적용 | kiko.ai 현재 | 개선 기회 |
|-----------------|----------|--------------|----------|
...
| 항목 | GABI 구현 | kiko.ai 적용 시 고려사항 | 우선순위 |
|------|----------|-------------------------|---------|
